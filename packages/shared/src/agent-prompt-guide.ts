...
export function buildWorkspaceAccessPlaybook(): string {
  return [
    ...
    '- 运行 python/node 脚本、pip/npm 安装依赖 → 必须经 opptrix_run / opptrix_install（系统隔离环境）；写自定义脚本后先 code_preflight；勿声称可读写未授权路径',
    '',
    '【消息内引用工作区文件】',
    ...
    '- opptrix_run / opptrix_install argv 只用字面量 python / python3 / pip（或 node/npm）；禁止手写系统或 Opptrix 托管绝对路径；运行时会静默改写到当前优先解释器',
    '- install 与 run 共用同一解释器；pip 依赖装进工作区 .opptrix-packages，运行时自动经 PYTHONPATH 可见',
    '- 依赖安装用 opptrix_install(manager=pip|npm)；pip 镜像由设置注入；python 未就绪时先 ensure_python（未就绪立即 preparing|installing+suggested_wake_seconds → 优先 schedule_turn_wake 到期再查，勿 tight-poll）',
    ...
    '- 预估需 pip/npm 安装 → 先 request_shell_network({ intent: "install" })，再 opptrix_install；走系统沙盒确认，禁止用 ask_user「允许联网」冒充（ask_user 不会写入沙盒授权）',
    ...
    '- 写自定义脚本：workspace_write（新建）→ code_preflight → 按 diagnostics 的 L 行号用 workspace_replace_lines 定点修 → 再 code_preflight → 通过后再 opptrix_run（软门禁，不硬拦）；禁止小改动却整文件 workspace_write',
    ...
    '',
    '【能力不足时的沙盒兜底】',
    '- 内置/已匹配工具无法完成、或没有匹配工具时：若尚未加载 workspace → activate_tool_pack([\'workspace\'])，再用 workspace_write → code_preflight → workspace_replace_lines → code_preflight → opptrix_run / opptrix_install / ensure_python 等编程实现',
...
export function buildLocalProgrammingPlaybook(): string {
  return [
    ...
    '4. 自写脚本：新建用 workspace_write；随后 code_preflight（尽量带 line / errors 前缀 L{n}:）→ workspace_replace_lines 按行号批量改 → 再 code_preflight → 通过后再 opptrix_run；禁止小改动却整文件 rewrite；可复用产物写入 shared/packages/<name>/ + README；workspace_write 默认 LF，.bat/.cmd/.ps1 用平台换行；脚本用 pathlib/path，禁止硬编码本机用户路径',
...
export function buildAgentSystemRules(opts?: AgentSystemRulesOptions): string {
  ...
  const hasShellTools = opts?.activeToolNames?.some(
    name => name === 'opptrix_run' || name === 'shell_run' || name === 'opptrix_install' || name === 'shell_install' || name === 'code_preflight'
  ...
  if (hasShellTools) {
    sections.push(
      ...
      '- opptrix_run argv 只用字面量 node/python/npm/pip；禁止手写系统/托管绝对路径；依赖：先 request_shell_network({intent:"install"}) 再 opptrix_install(pip|npm)；禁止 ask_user 冒充沙盒联网授权；install 与 run 同一解释器与 .opptrix-packages；禁止 powershell/cmd/bash -c 整串绕过；darwin/linux ping 用 -c，win32 用 -n 且 tracert 替代 traceroute',
      '- 预估需访问已知外网域名 → request_shell_network({intent:"egress", hosts:[...]})；局域网仍用 request_session_lan_access',
      '- workspace_write 文本默认 LF；.bat/.cmd/.ps1 用平台换行；脚本用 pathlib/path，禁止硬编码本机用户路径；写脚本后 code_preflight → workspace_replace_lines（按 L 行号）→ 再 code_preflight → opptrix_run；禁止小改动却整文件 rewrite',
      ...
      '- 编程：list_local_data_apis → get_local_data_catalog → 复用 shared/packages → request_shell_network(install) → opptrix_install → workspace_write → code_preflight → workspace_replace_lines → code_preflight → opptrix_run；离线 dump 用 prepare_fuyao_dump',
    ...
    sections.push(
      '- 未加载 opptrix_run / workspace_* 时：勿声称具备本地命令、工作区文件或未提供的工具能力；内置工具不够或无匹配时 → activate_tool_pack([\'workspace\'])，用 opptrix_run / opptrix_install / code_preflight / ensure_python / workspace_* 沙盒编程实现（可先标准工具取数再沙盒计算），勿空转 activate 无关 pack，勿直接声称无法完成',
    )
  }
