...
export function buildWorkspaceAccessPlaybook(): string {
  return [
    ...
    '- 领域工具仅用于行情/财务/资讯/画布等特色；禁止用 evaluate_instrument / get_instrument_* 等代替文件或脚本操作；禁止用 opptrix_run 爬网页或平行造行情数据源',
    ...
    '- opptrix_run 传 command；python / python3 / pip / node / npm 字面量会静默改写到当前优先解释器（含真 shell 管道/&& 时亦同步到 commandString）；禁止手写系统或 Opptrix 托管绝对路径',
    '- 安装与运行共用同一解释器；pip 依赖装进工作区 .opptrix-packages，运行时自动经 PYTHONPATH 可见',
    '- 依赖：直接 opptrix_run(command="pip install …" 或 npm)；包源默认已放行。ensure_python 是失败兜底（未就绪 preparing|installing+job_id → 系统通常自动挂起，完成后通知续跑；无 job 事件才 schedule_turn_wake 且禁止传 job_id；勿 poll/sleep 查进度），不是编程第一步',
...
export function buildLocalProgrammingPlaybook(): string {
  return [
    ...
    '5. 离线大数据 → prepare_fuyao_dump（冷下载先 preparing+job_id，系统通常自动挂起并完成后通知续跑；无 job 事件时才 schedule_turn_wake 且禁止传 job_id）；行情优先标准工具；禁止明文密钥进沙盒（经保险箱 + secret_refs 注入 sentinel）；勿引导 sync/dailyDump',
...
export function buildUserInteractionPlaybook(): string {
  return [
    ...
    '- 业务工具返回 preparing|accepted|installing+job_id（如 prepare_fuyao_dump / ensure_python），或 opptrix_run({ background:true })：系统通常自动挂起，任务完成后同会话自动通知续跑；禁止 poll / watch / sleep / 反复查进度',
...
export function buildAgentSystemRules(opts?: AgentSystemRulesOptions): string {
  ...
  if (hasShellTools) {
    sections.push(
      ...
      '- 编程：list_local_data_apis → get_local_data_catalog → 复用 shared/packages（优先 workspace_glob）→ opptrix_run 安装依赖 → glob/grep → read → replace_lines / write → code_preflight → opptrix_run；探树：list_workspace_grants 至多一次 → workspace_glob 或相对 cwd 的 opptrix_run(ls/find)；离线 dump 用 prepare_fuyao_dump',
