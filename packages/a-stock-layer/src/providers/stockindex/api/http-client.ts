...
export class StockIndexHttpClient extends ProviderHttpClient {
  ...
  private errorFrom(status: number, body: string): OpptrixQuantApiError {
    ...
    if (status === 429) {
      return new OpptrixQuantApiError(
        message ? `OpptrixQuant 配额已用尽：${message}` : 'OpptrixQuant 配额已用尽（HTTP 429）',
        status,
        code,
      )
    }
    return new OpptrixQuantApiError(
      message ? `OpptrixQuant API ${status}：${message}` : `OpptrixQuant HTTP ${status}`,
      status,
      code,
    )
  }
  ...
  private async requestJson<T>(
    ...
    if (!text.trim()) throw new OpptrixQuantApiError('OpptrixQuant 返回空响应体', resp.status)
