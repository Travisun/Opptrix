...
export const YFINANCE_SPEC: ProviderManifestSpec = {
  id: 'yfinance',
  title: 'Yahoo 财经',
  subtitle: 'Yahoo Finance · 代用户浏览（美/港/日/韩行情 · 限速 2 秒/次，无并发）',
  marketGroup: 'GLOBAL',
  defaultPriority: 46,
  capabilities: YFINANCE_CAPS,
  bindingsFor: (p) => [
    ...usEquityBindings(US_CAPS, p),
    ...regionalEquityBindings('HK', REGIONAL_CAPS, p),
    ...regionalEquityBindings('JP', REGIONAL_CAPS, p),
    ...regionalEquityBindings('KR', REGIONAL_CAPS, p),
    ...cnEquityBindings([Capability.GLOBAL_INDEX], p),
  ],
