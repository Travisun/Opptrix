...
import { isTushareEnabled, loadTushareConfig } from '../tushare/config.js'
import { TushareClient, type TushareRow } from '../tushare/client.js'
import { fromTsCode, indexTsCode, toTsCode } from '../tushare/codes.js'
import {
  latestOpenTradeDate,
  mapDailyQuoteRows,
  mapDividendRows,
...
export class TushareDriver extends BaseDriver {
  private nameCache = new Map<string, string>()
  private snapshotCache: { tradeDate: string; quotes: Map<string, StockRealtime> } | null = null

  get name() { return 'tushare' }

  get priority() {
    return isTushareEnabled() ? ENABLED_PRIORITY : DISABLED_PRIORITY
  }
