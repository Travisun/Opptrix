...
describe('tencent HK stock list API', () => {
  it('resolves board and maps hk rank page_data rows', async () => {
    ...
    expect(resolveTencentHkBoard('hk_mb_hscei')).toBe('main_China')
    expect(resolveTencentHkBoard('MBHSCCI')).toBe('main_red')
    expect(resolveTencentHkBoard('hk_cbbc_pull')).toBe('niuxiong_niu')
    expect(resolveTencentHkBoard('BEAR')).toBe('niuxiong_xiong')
