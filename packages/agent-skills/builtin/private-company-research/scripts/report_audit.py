#!/usr/bin/env python3
"""Report Audit Tool — Opptrix financial-data skill (stdlib only).

数据抽检工具：从研究报告中抽取 15% 的财务数据点，与可靠信源比对，
通过则准出，不通过则打回并说明原因。

主路径取数：Opptrix 工具（财务三表 / 公告 / 机构研报 / 用户 panels），
禁止把外网爬虫当作主路径。
...
工作流程（三步）：
  ...
  Step 2 — Agent 对抽检清单中的每个数据点，用 Opptrix 财务/公告工具或第二源 panels 取数，
            填入 fetched_value
...
工作流程：
  ...
  Step 2 — Agent 对清单中每个数据点，用 Opptrix 财务工具或第二源 panels 取数，
  ...
  固定随机种子（复现同一批样本）：
    ...
    if args.command == 'extract':
        ...
        print('  主路径：Opptrix get_instrument_financials / 三表 / financial_indicators / snapshot')
        print('  第二源：get_instrument_notices / get_instrument_institution_report，或用户 panels 双源')
