...
const INTENT_RULES: IntentRule[] = [
  ...
  },
  {
    ...
    preferredTools: ['get_cn_market_special', 'get_market_dynamics', 'get_sector_list'],
    avoidTools: ['get_instrument_snapshot', 'evaluate_instrument'],
    confidence: 'high',
    hint: '连板天梯/热股/异动/同花顺概念指数 → get_cn_market_special(kind=…)；全景涨跌榜仍用 get_market_dynamics；标准行业目录用 get_sector_list',
  },
  {
    intent: 'financials',
    priority: 72,
    patterns: [/营收|净利润|ROE|财报|财务|同比|毛利率|每股收益|\bEPS\b|利润表/i],
    preferredTools: ['get_instrument_financials', 'get_instrument_balance_sheet', 'get_instrument_cash_flow', 'get_instrument_snapshot'],
    avoidTools: ['evaluate_instrument', 'invoke_provider_custom_method'],
    confidence: 'high',
    hint: '财务摘要 → get_instrument_financials；资产负债表/现金流表明细改用专用工具；详细能力指标可再 activate provider_ext 调 thsFinancialIndicators',
  },
  {
    intent: 'profile',
    priority: 70,
  ...
  },
  {
    intent: 'provider_ext',
    priority: 56,
    patterns: [/自定义方法|invoke_provider|akshare|baostock|list_provider|财务指标明细|thsFinancialIndicators/i],
    preferredTools: ['list_enabled_providers', 'list_provider_custom_methods', 'invoke_provider_custom_method'],
    avoidTools: ['get_instrument_snapshot'],
    confidence: 'medium',
    hint: '自定义数据源 / 同花顺财务指标明细 thsFinancialIndicators → list → invoke；标准资产负债/现金流勿走 custom',
  },
  {
    ...
    avoidTools: ['get_instrument_quotes'],
    confidence: 'medium',
    hint: '深度分析：已知代码可跳过 search → snapshot → financials/BS/CF/profile 事实表 → evaluate；仅报价不够',
  },
  {
    intent: 'etf_general',
    priority: 38,
