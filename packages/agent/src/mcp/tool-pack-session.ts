...
export function unloadedToolHint(toolName: string): string {
  ...
  return `未知或不支持的工具：${toolName}。请先 list_tool_packs 查看可用工具包；若无合适 pack → activate_tool_pack，参数 pack_ids: ["workspace"]，用 shell_run / ensure_python / workspace_* 沙盒编程实现，勿虚构工具名。`
}
