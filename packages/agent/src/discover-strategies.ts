...
export const DISCOVER_STRATEGIES: DiscoverStrategy[] = [
  cnEquity({
    ...
    refinement_notes: '优先低波动、回撤可控的价值型标的；回避 ST 与高投机壳股；匹配度需体现安全边际与趋势稳定性。',
  }),
  cnEquity({
    id: 'buffett_moat',
    name: '巴菲特护城河',
    ...
    methodology: '参考 Buffett 四透镜模型（业务简单、护城河、资本配置、估值）；本地因子以中期动量+低波动+温和回撤近似质量龙头。',
    ...
    refinement_notes: `优先趋势连贯、波动温和、回撤可控的龙头；区分周期景气与真护城河。${WATCHLIST_GATE}`,
  }),
  cnEquity({
    id: 'lynch_garp',
    name: '林奇 GARP',
  ...
  cnEquity({
    ...
    conditions: [
      C('momentum_6m', '>=', 0),
      C('volatility_20d', '<=', 3.5),
      C('drawdown_60d', '>=', -25),
      C('momentum_3m', '>=', -3),
    ],
    refinement_notes: '强调趋势稳定与波动可控；优先市值与流动性较好、行业龙头；关注动量改善而非单周脉冲。',
  }),
  cnEquity({
  ...
  cnEquity({
    ...
    refinement_notes: '区分一次性脉冲与趋势加速；优先量价同步改善的标的。',
  }),
  cnEquity({
    id: 'smart_flow',
    name: '量能共振',
