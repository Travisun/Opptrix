...
export const TOOL_META: Record<string, ToolMeta> = {
  ...
  activate_agent_skill: {
    packId: 'meta',
    usageGuide: '激活工作流技能，将完整步骤注入本会话 system；用户提到早报/财报速读/深度分析流程时使用。',
    compliance: 'skill_names 为字符串数组；同会话最多 3 个；无效名或超额进入 skipped。',
  },
  get_agent_skill: {
    packId: 'meta',
  ...
  create_agent_skill: {
    packId: 'meta',
    usageGuide: '为用户创建新的工作流技能；必须先 ask_user 确认，再 confirmed=true。',
    compliance: 'name/description/body 必填；未 confirmed 只返回摘要；名称须小写连字符。',
  },
  import_agent_skill: {
    packId: 'meta',
