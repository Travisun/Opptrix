...
export const TOOL_META: Record<string, ToolMeta> = {
  ...
  create_canvas: {
    miningEligible: false,
    usageGuide:
      '用户明确点名可视化报告/投研画布，或本轮已自感应决定交付完整多章节图文报告时创建（禁止先 ask_user 问是否出报告）；日常「画个图」用消息 ```chart 围栏，勿误用本工具。图表勿全宽拉满。默认机构调研报告版式（H1→导语→H2 分章→正文与 Chart/Table/Stat 穿插）；定量对比/变化/构成/强弱矩阵优先 Chart（bar/line/pie/heatmap）+ 主题配色，Table/Stat 作明细与 KPI；Chart 默认居中、max-width bar/line~320 pie~230 heatmap~380，勿写 width:100% / 超大 height；图注用 Chart caption（与图居中对齐），勿全宽左对齐旁白；Chart 已含轴/网格/数值标注，勿手写假坐标；避免 Divider；章节靠标题与 Stack；仅用户明确要求时例外；勿用 Card 墙做面板分割；须含介绍与说明文字；仅用户明确要面板/仪表盘时才用密面板布局；语义配色：文字层级用 Text tone（primary/secondary/tertiary）；涨跌默认红涨绿跌（danger/success）；tips/风险用 Callout（tone+可选 variant）；原文/口径摘录用 Quote（cite 来源），勿用 Callout 冒充引用；行内 Pill/Code/Link；可见文案禁止 emoji；返回 attachment 供消息内预览。',
    compliance:
