...
3. 每项附证据（命令输出 / 字段值）
4. 本地构建 / `check:ui` 能跑则跑；失败禁止打标签
5. 打标签前向用户汇报；未明确「直接发布」则待确认
6. **禁止**：未改 version / 未写更新日志 / 标签与 version 不一致就 tag
...
C1 schema/存储迁移；C2 API 双读/版本化；C3 更新源不断链；C4 日志覆盖用户可见修复；C5 Electron 升级说明。
...
E1 workflow 成功；E2 三端产物 + `latest-*.yml`；E3 verify-packaged-updater；E4 Release 正文含新功能/修复；E5 R2（若启用）。**禁止**手动改名产物或 yml 内 url/version。
...
状态：`user-store` → `onboarding_state`（`GET/PUT /api/preferences/onboarding_state`）。
