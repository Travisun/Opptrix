...
| 付费 / SDK 自限流 | `tushare` / `tonghuashun` | `ProviderHttpClient` + `bypassRateLimit: true` | 否 |
| SDK 自限流 | `tickflow`（`@opptrix/tickflow-sdk` `intervalMs`） | **不经** `ProviderHttpClient` / 主机闸门 | 有 Key 否；无 Key 仍可走阶梯冷却（若错误上抛） |
| 付费可 bypass | `tushare` / `tonghuashun` | `bypassRateLimit: true` | 否 |
