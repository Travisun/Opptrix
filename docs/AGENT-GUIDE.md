...
- **与出站关系**：LAN 仅放宽私网/localhost **连接判定**；具体域名仍可能需 `network_egress` 确认（`http_fetch` / `opptrix_run` ping 等读有效 LAN）。
  ...
  - **公募基金（CN:PF）**：`get_fund_list` / `get_fund_profile` / `get_fund_nav` / `get_fund_holdings`；须 `assetClass=FUND` 命名空间（兼容 `CN:OF`），勿与 ETF 行情工具混用。桌面详情页另经 Hub `fund_detail` 聚合区间收益/回撤/配置/持有人/分红（非 MCP 工具）
