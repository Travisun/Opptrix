...
- **工具包路由（Tool Pack Router）**：
  ...
  - **工作区与文件（`workspace` pack）**：实现 `@opptrix/agent-workspace` + `packages/agent/src/mcp/workspace-tools.ts`
    ...
    - **编程协议**：查目录 → 扫 `shared/packages` → `request_shell_network({intent:"install"})` → `shell_install` → 自写回写 README；离线大数据用 `prepare_fuyao_dump`（服务端持 Key 落盘 `shared/data/dumps` 或短时效 URL）；**禁止** Key 进沙盒；**禁止**引导 `market sync` / `dailyDump`。**禁止**用 `ask_user`「允许联网」冒充沙盒联网授权（须 `request_shell_network` / ConfirmHandler）。
...
4. 最后自写；可复用产物写入 `shared/packages/<name>/` + README（目的/入口/入参出参/依赖/示例/勿存密钥）；`workspace_write` 默认 LF，`.bat`/`.cmd`/`.ps1` 用平台换行
