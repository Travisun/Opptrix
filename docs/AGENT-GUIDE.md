...
- **工具包路由（Tool Pack Router）**：
  ...
  - **画布与脑图（`artifacts` pack）**：实现 `packages/agent/src/canvas-tools.ts`；非 always-on，意图播种（可视化报告/画布/脑图/思维导图）或 `activate_tool_pack({ pack_ids: ["artifacts"] })`
    ...
    - **何时用**：个股/市场投研或数据分析解读时，优先 `ask_user` 询问是否生成可视化报告（用户已点名报告/画布或 L1 快答可跳过）；同意后加载 `artifacts` 并以 `create_canvas` 为主交付。用户要可预览的可视化报告、投研画布 → `create_canvas`；脑图/思维导图/结构化主题树 → `create_mindmap`；更新先 `read_*` 再 `update_*`
