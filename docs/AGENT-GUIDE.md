...
- **与出站关系**：LAN 仅放宽私网/localhost **连接判定**；具体域名仍可能需 `network_egress` 确认（`http_fetch` / `shell_run` ping 等读有效 LAN）。
  ...
  - **会话时钟 / 前缀缓存**：Engine 每轮将 `getCurrentTime()`（Asia/Shanghai）写入**本轮 turn-tail**（messages 末尾 ephemeral user），**不**写入稳定 system，以免破坏 DeepSeek 等前缀缓存；选型卡同理。`get_current_time` 仅在用户明确问时刻时调用。未显式定制时输出额度 ladder：普模 16k、推理 32k、`reasoningEffort=high` 为 64k（显式等于历史默认 4096 会抬升；更低显式值仍尊重）。上游 `reasoning_content` 会累积；工具轮写入会话并在下一请求 wire 回传（含空串占位）；终轮仅非空思考写入；空正文时提示思考占用输出上限。
...
- **会话上下文管理（长对话压缩）**：实现 `packages/agent/src/context/*` + `llm/model-context.ts`。
  ...
  - **会话 LLM 参数**：`SessionRecord.llmParams`（`temperature` / `maxTokens` / `reasoningEffort`）经 `PATCH /api/sessions/:id` 持久化；聊天请求体优先会话覆盖，缺省温度 1；`max_tokens` 未设或仍为历史默认 4096 时按 ladder 抬升（普模 16k），未设则不发 `reasoning_effort`。
