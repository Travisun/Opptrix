...
- **工具包路由（Tool Pack Router）**：
  ...
  - **市场（`market` pack）**：`get_market_dynamics`（全景）；`get_macro_series`（CPI/PPI/PMI/GDP/LPR 等事实序列）；专项 `get_dragon_tiger` / `get_limit_updown` / `get_market_sentiment`；同花顺独有 `get_cn_market_special`；`get_trade_calendar` / `get_market_session`；`get_instrument_money_flow`
