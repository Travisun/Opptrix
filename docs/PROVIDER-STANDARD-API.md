...
| **tonghuashun** | ✅ | CN；**CN ETF 分拆**（`CN_ETF_CAPABILITIES`，priority 120）；含 BALANCE_SHEET / CASH_FLOW | CN | ✅ ETF_LIST/PROFILE/NAV/HOLDINGS + STOCK_REALTIME/KLINE | ✅ | `ths*` 指数/特色数据（9）；Fuyao `/api/fund/*` 场内 ETF 标准路径 | **合规（CN 个股 + ETF）** |
