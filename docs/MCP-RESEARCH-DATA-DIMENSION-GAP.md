...
| `sector_list` | 板块/行业目录 | 无直接工具；产业靠 `industry_mining` 叙事 |
| `etf_profile` | ETF 档案（跟踪指数、费率等） | 无；仅 list/nav/holdings |
| `news` / `notices`（标的绑定） | 个股新闻/公告列表 | 部分靠 snapshot 内嵌 + 通用 RSS；**缺「按标的拉官方公告列表」** |
| `stock_list` | 全市场名录分页 | Agent 用 search，缺「按板块/行业筛名录」工具 |
...
| `get_market_calendar` | `core` 或 `market` | 交易日/休市/时段（CN/US/HK） |
...
| 建议工具名 | Pack 建议 | 说明 |
|------------|-----------|------|
| `get_sector_list` / `get_sector_constituents` | `industry` 扩展 | `sector_list` + 成分；支撑 peer |
| `compare_instruments` | `instrument_analytics` | 封装 batch snapshot + 关键财务列对齐；非新数据源 |
...
| `get_etf_profile` | `etf` | 跟踪指数、费率、规模 |
...
| `market` | 扩展 | + calendar, macro_series |
