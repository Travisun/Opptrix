...
test('settings — 默认基地址为 quant.opptrix.net', () => {
  ...
  assert.equal(STOCKINDEX_SETTINGS.title, 'OpptrixQuant')
})

test('settings — apiKey 为必填 secret 且 enabled 默认 false', () => {
  ...
  assert.equal(baseUrl?.default, 'https://quant.opptrix.net')
  assert.equal(providerRequiresApiKey(STOCKINDEX_SETTINGS.fields), true)
})

test('settings — 旧域名（lirdb / cuishushu）迁移到 quant.opptrix.net', () => {
  ...
  for (const legacy of [
    ...
    process.env.OPPTRIX_STOCKINDEX_BASE_URL = legacy
    assert.equal(stockIndexBaseUrl(), 'https://quant.opptrix.net', `legacy=${legacy}`)
  }
...
test('manifest — 绑定含 CN FUND 三能力、CN ETF ETF_LIST、不含 SECTOR_LIST', () => {
  const bindings = STOCKINDEX_SPEC.bindingsFor(92, 4)
