...
/** Hybrid RAG natives — must load under ELECTRON_RUN_AS_NODE (same as packaged sidecar). */
const RAG_NATIVE_REQUIRED = ['sharp', 'onnxruntime-node', '@lancedb/lancedb']
const RAG_NATIVE_OPTIONAL = ['@gutenye/ocr-node']
...
function ensureStageNodeModulesLink() {
  ...
  const linkTarget = path.join(STAGE, RUNTIME_DEPS_DIR)
  if (!fs.existsSync(linkTarget)) fail(`missing ${linkTarget} for node_modules link`)
  try {
    if (process.platform === 'win32') {
      fs.symlinkSync(linkTarget, nm, 'junction')
    } else {
...
function resolveElectronBinary() {
  ...
  try {
    return resolveElectronExecutable()
  } catch { /* fall through to packaged app */ }
...
function packageJsonInDeps(modName) {
  return path.join(depsRoot, ...modName.split('/'), 'package.json')
}

/**
 * Assert Hybrid RAG natives are staged and (when host matches target) importable
 * under Electron ELECTRON_RUN_AS_NODE — same ABI path as packaged sidecar.
 * Never silently skip missing packages (that would ship knowledge-base regressions).
 */
function assertRagNativeImports() {
  for (const mod of RAG_NATIVE_REQUIRED) {
    if (!fs.existsSync(packageJsonInDeps(mod))) {
      fail(
        `missing RAG native package ${mod} under ${depsRoot} — `
          + 're-run stage-runtime.mjs so @opptrix/doc-library (and its deps) are installed into the sidecar stage. '
          + 'Packaging without these modules degrades Hybrid RAG / knowledge-base features.',
      )
    }
    console.log(`verify-runtime: OK staged ${mod}`)
  }

  const optionalPresent = []
  for (const mod of RAG_NATIVE_OPTIONAL) {
    if (fs.existsSync(packageJsonInDeps(mod))) {
      optionalPresent.push(mod)
      console.log(`verify-runtime: OK staged optional ${mod}`)
    } else {
      console.log(
        `verify-runtime: optional ${mod} not in staged deps tree — skip import `
          + '(ok if resolved transitively under another package name)',
      )
    }
  }

  const toImport = [...RAG_NATIVE_REQUIRED, ...optionalPresent]

  if (!hostMatchesTarget(target)) {
    console.log(
      `verify-runtime: skip RAG native import smoke `
        + `(host ${process.platform}-${process.arch} != staged ${target.platform}-${target.arch}) `
        + '— package trees present; live import requires matching Electron ABI',
    )
    return
  }

  // Smoke script MUST live under STAGE so bare ESM imports walk up to
  // STAGE/node_modules → deps/ (NODE_PATH is ignored for ESM bare specifiers).
  // Writing under /tmp makes `import('sharp')` fail with ERR_MODULE_NOT_FOUND
  // even when the package is correctly staged.
  const smokeDir = path.join(STAGE, '.verify-rag-native')
  fs.rmSync(smokeDir, { recursive: true, force: true })
  fs.mkdirSync(smokeDir, { recursive: true })
  const smokeScript = path.join(smokeDir, 'import-smoke.mjs')
  fs.writeFileSync(
    smokeScript,
    [
      '// generated by verify-runtime.mjs — keep under runtime-stage for module resolution',
      `const mods = ${JSON.stringify(toImport)};`,
      '(async () => {',
      '  for (const m of mods) {',
      '    try {',
      '      await import(m);',
      "      console.log('OK import', m);",
      '    } catch (err) {',
      "      console.error('FAIL import', m, err instanceof Error ? (err.stack || err.message) : err);",
      '      process.exit(1);',
      '    }',
      '  }',
      '})();',
  ...
  const nodePath = [depsRoot, path.join(STAGE, 'node_modules')]
    .filter((p, i, a) => a.indexOf(p) === i)
    .join(path.delimiter)

  const result = spawnSync(electronBin, [smokeScript], {
    cwd: STAGE,
    env: {
      ...process.env,
  ...
  if (result.status !== 0) {
    ...
    fail(
      `RAG native import failed under ELECTRON_RUN_AS_NODE `
        + `(${toImport.join(', ')}). Sidecar knowledge-base will regress.\n`
        + (detail || `exit ${result.status ?? 'null'}${result.error ? `: ${result.error.message}` : ''}`),
    )
  }
  console.log(`verify-runtime: OK RAG native imports (${toImport.join(', ')})`)
}

assertRagNativeImports()
